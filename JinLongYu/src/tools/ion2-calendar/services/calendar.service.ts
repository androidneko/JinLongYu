...
import { CnCalendarService } from './cnCalendar.service';

@Injectable()
export class CalendarService {

  constructor(public cnCalendar:CnCalendarService) {
  ...
  createCalendarDay(time: number, opt: CalendarModalOptions, month?: number): CalendarDay {
    ...
    let cnLunar = "";
    let cnLunarInfo:any = this.cnCalendar.solar2lunar(theDay.getFullYear(),theDay.getMonth()+1,theDay.getDate());
    //按照农历下标重要性次序，依次展示农历信息，包含节日，节气等,初一优先显示月份
    if (cnLunarInfo.IDayCn){
      cnLunar = cnLunarInfo.IDayCn;
    }
    if (cnLunarInfo.IDayCn == "\u521d\u4e00"){
      cnLunar = cnLunarInfo.IMonthCn;
    }
    if (cnLunarInfo.isTerm){
      cnLunar = cnLunarInfo.Term;
    }
    if (cnLunarInfo.lunarFestivals){
      cnLunar = cnLunarInfo.lunarFestivals;
    }
    if (cnLunarInfo.calendarFestivals){
      cnLunar = cnLunarInfo.calendarFestivals;
    }

    //通过day config使用自定义tag属性来配置当前天是否为月份最后一行
    let lastLine = false;
    if (dayConfig && dayConfig.tag && dayConfig.tag === true) {
      lastLine = true;
    }
