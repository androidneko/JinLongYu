...
export class CalendarComponent implements AfterViewInit, ControlValueAccessor, OnInit {
  ngAfterViewInit(): void {
    console.log("ngAfterViewInit....");
  }
  @ViewChild(MonthComponent) monthComp:MonthComponent;
  ...
  monthOnSelect(month: number): void {
    ...
    let oldM = this.calSvc.multiFormat(this.monthOpt.original.time);
    let newM = this.calSvc.multiFormat(newMonth);
    //先创建当月日视图，再向上回调月分选择事件
  ...
  /**
     * @argument year,which year you want to go,must >= 1970
     */
  selectYear(year:number){
    if (year < 1970 ) return;
    let origYear = moment(this.monthOpt.original.time).year();
    if (origYear === year) return;
    if (origYear > year){
      if (origYear === 1970) return;
      let dis = origYear-year;
      const backTime = moment(this.monthOpt.original.time).subtract(dis, 'year').valueOf();
      this.monthOpt = this.createMonth(backTime);
    }else {
      let dis = year-origYear;
      const nextTime = moment(this.monthOpt.original.time).add(dis, 'year').valueOf();
      this.monthOpt = this.createMonth(nextTime);
    }
    
  }

  selectMonth(month:number,from:number){
    this._view = 'days';
    const newMonth = moment(this.monthOpt.original.time).month(month).valueOf();
    let oldM = this.calSvc.multiFormat(this.monthOpt.original.time);
    let newM = this.calSvc.multiFormat(newMonth);
    ...
    //from:定义了这个月份选择改变得来源，默认0为原日历滑动或月份选择器触发得月份选择变化；1为新增自定义方法today触发得月份选择
    this.monthChange.emit({
      oldMonth: oldM,
      newMonth: newM,
      from:from
  ...
  selectDay(date:Date){
    let day = new Date(date.getFullYear(),date.getMonth(),date.getDate(),0,0,0);
    let calendarDay:CalendarDay = {time:day.getTime(),disable:false,isToday:true,selected:true,cssClass:""};

    if (this.monthComp){
      this.monthComp.onSelected(calendarDay);
    }
  ...
  today(){
    let today = new Date();
    let origYear = moment(this.monthOpt.original.time).year();
    let origMonth = moment(this.monthOpt.original.time).month();

    if (today.getFullYear() == origYear && today.getMonth() == origMonth){
      //年份和月份都无需变化，则直接选择当天
      this.selectDay(today);
    }else{
      if(today.getFullYear() != origYear){
        //年份有变化，首先调整日历年份，将年份先跳转到今年
