      ...
      <ng-template [ngIf]="!_isRange" [ngIfElse]="rangeBox">
        <div class="days-box">
          <ng-template ngFor let-day [ngForOf]="month.days" [ngForTrackBy]="trackByTime">
            <div class="days">
              <ng-container *ngIf="day && !day.isLastMonth && !day.isNextMonth">
                <button type='button'
                        ...
                        [disabled]="day.disable||day.isLastMonth||day.isNextMonth">
                  <p>{{day.title}}</p>
                  <p style="font-size:1.0rem;">{{day.cnLunar}}</p>
                  <small *ngIf="day.subTitle" style="margin-top:3px">{{day?.subTitle}}</small>
                </button>
              </ng-container>
              <div *ngIf="day && !day.isLastMonth && !day.isNextMonth && !day.isLastLine" style="width:100%;height:1px;background-color: #eeeeee;margin-top:8px;"></div>
            </div>
          </ng-template>
        </div>
      </ng-template>
...
export class MonthComponent implements ControlValueAccessor, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this._isInit = true;
    console.log("ngAfterViewInit.MonthComponent...");
  }
