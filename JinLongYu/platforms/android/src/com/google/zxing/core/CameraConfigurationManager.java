...
final class CameraConfigurationManager {
	...
	void initFromCameraParameters(Camera camera) {
		...
		LogU.d(TAG, "Default preview format: " + previewFormat + '/'
				+ previewFormatString);
		WindowManager manager = (WindowManager) context
				.getSystemService(Context.WINDOW_SERVICE);
		Display display = manager.getDefaultDisplay();
		screenResolution = new Point(display.getWidth(), display.getHeight());
		LogU.d(TAG, "Screen resolution: " + screenResolution);
		...
		LogU.d(TAG, "Camera resolution: " + screenResolution);
	}
	...
	private static Point getCameraResolution(Camera.Parameters parameters,
		...
		if (previewSizeValueString != null) {
			LogU.d(TAG, "preview-size-values parameter: "
	...
	private static Point findBestPreviewSizeValue(
		...
		for (String previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
			...
			if (dimPosition < 0) {
				LogU.d(TAG, "Bad preview-size: " + previewSize);
				continue;
			}
	...
	private void setZoom(Camera.Parameters parameters) {
		...
		if (maxZoomString != null) {
			try {
				...
				LogU.d(TAG, "Bad max-zoom: " + maxZoomString);
			}
		}
		...
		if (takingPictureZoomMaxString != null) {
			try {
				...
				LogU.d(TAG, "Bad taking-picture-zoom-max: "
	 ...
	 * (NumberFormatException nfe) { LogU.d(TAG, "Bad sharpness-max: " +
	 * maxSharpnessString); } }
	 *
	...
	private static String findSettableValue(Collection<String> supportedValues,
		...
		LogU.d(TAG, "Supported values: " + supportedValues);
		String result = null;
		...
		LogU.d(TAG, "Settable value: " + result);
		return result;
	}
}
