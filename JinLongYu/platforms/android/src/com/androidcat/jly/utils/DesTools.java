...
public final class DesTools {
  ...
  //3des加密
  public static String encrypt3des(String Mpikey, String data) {
    try {
      // 判断加密还是解密
      ...
      Cipher enc = null;
      if (Mpikey.length()<24) {
        Mpikey = Mpikey+"00000000000000000000000000000000";
      }
      // 计算加解密密钥，即将16个字节的密钥转换成24个字节的密钥，24个字节的密钥的前8个密钥和后8个密钥相同,并生成安全密钥
      keySpec = new SecretKeySpec(Mpikey.substring(0, 24).getBytes("utf-8"), "DESede");// 将key前8个字节复制到keyecb的最后8个字节
      // 生成算法
      enc = Cipher.getInstance("DESede/ECB/NoPadding");
      // 初始化
      enc.init(opmode, keySpec);
      data = encodeHexString(data);
      LogU.e("encypt","encodeHexString:"+data);
      data = padding80(data);
      LogU.e("encypt","padding80:"+data);
//      try {
//        Base64.encodeToString(enc.doFinal(data.getBytes("utf-8")),Base64.DEFAULT);
//      } catch (Exception e) {
//        e.printStackTrace();
//      }
      // 返回加解密结果
      return Base64.encodeToString(enc.doFinal(data.getBytes("utf-8")),Base64.NO_WRAP);// 开始计算
  ...
  //3des解密
  public static String decrypt3des(String Mpikey, String data) {
    try {
      ...
      data = new  String(enc.doFinal(Base64.decode(data,Base64.NO_WRAP)));
      int num = data.lastIndexOf("80");
      // 截取actionInfoString
      // 最后一个'80'出现的位置
      if (num != -1) {
      ...
      // 将16进制数字解码成字符串,适用于所有字符（包括中文）
      data = SecurityUtil.hexStringToString(data);
      // 返回加解密结果
      return data;
    } catch (Exception e) {
    }
