...
public class StringUtils {
	...
	public static String getLeftZeroNumString(String number, int width) {
		try {
			int len = width - number.getBytes(UTF_8).length;
	...
	public static String getRightBlankString(String str, int width) {
		try {
			int len = width - str.getBytes(UTF_8).length;
	 ...
	 * 改为
	 * int len = width - bcdString.getBytes(HTTP.UTF_8).length;
	 * @param number
	 * @param width 总长度
	 * @return
	 */
	public static String getFormatedNumBcdString(int number, int width) {
		...
		try {
			int len = width - bcdString.getBytes(UTF_8).length;
			for (int i = 0; i < len; i++) {
	...
	public static boolean beoreEndTime(String str, String endTime) {
		...
		LogU.d(str1 + "_" + str2 + "_" + str3);
		LogU.d(endTime1 + "_" + endTime2 + "_" + endTime3);
		if (str1 < endTime1) {
			return true;
   ...
   * @return
   */
  public static File base64ToFile(String base64,String fileType) {
    File file = null;
    String fileName = "/.ionic/.file/file"+fileType;
