package com.androidcat.jly.persistense;
...
public class JepayDatabase {
	...
	private static void upgradeDatabase(DbUtils dbUtils, int oldVer, int newVer) {
		LogU.e(TAG, "数据库版本不一致，进入升级数据库流程");
		LogU.e(TAG, "当前数据库版本：" + oldVer + "  ----需升级到：" + newVer);
		try{
			...
			/*if (newVer == 2){
				...
				com.androidcat.utilities.LogU.e(TAG, "----删除旧版xx表，新增xx关联字段----");
			}*/
		}catch (Exception e){
